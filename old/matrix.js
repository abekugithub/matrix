...
async addMessageToUI(event, room, container = null) {
    ...
    async resolveMxcDownloadUrl(mxcUrl) {
        ...
        // Try v3 first with HEAD, fallback to r0
        try {
            const res = await fetch(v3, { method: 'HEAD' });
            if (res.ok) return v3;
        } catch (e) {
            console.warn('v3 download failed, using r0:', e);
        }
        return r0;
    }

    /**
     * Resolve MXC URL to HTTP thumbnail URL with v3/r0 fallback
     */
    async resolveMxcThumbnailUrl(mxcUrl, width = 800, height = 800, method = 'scale') {
        ...
        try {
            ...
            console.warn('v3 thumbnail failed, using r0:', e);
        }
        return r0;
    }
